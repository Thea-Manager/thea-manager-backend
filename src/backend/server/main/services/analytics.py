...
class Analytics:
    ...
    def get_project_analytics(self, token: str, customer_id: str, project_id: str):
        ...
        projection_expression = ", ".join(["scopes", "dataroom"])

        # Query DynamoDB request
        logger.info(f"Checking if project ID or organization ID exists: {query_key}")
        response = self._db.read_single_item(
            table_name, query_key, projection_expression
        )[0]
        ...
        new_response["scopes"]["status"] = {
            "labels": list(new_response["scopes"]["status"].keys()),
            "data": list(new_response["scopes"]["status"].values()),
        }

        ############################## report Analytics ##############################

        # report variables
        reports_overdue, reports_due, reports_status, due_dates, report_names = (
            0,
            0,
            [],
            [],
        ...
        new_response["reports"]["time"] = {
            "labels": list(new_response["reports"]["time"].keys()),
            "data": list(new_response["reports"]["time"].values()),
        }

        new_response["reports"]["status"] = {}
        new_response["reports"]["status"]["pending"] = len(
            list(filter(lambda x: "pending" in x, reports_status))
        )
        new_response["reports"]["status"]["accepted"] = len(
            list(filter(lambda x: "accepted" in x, reports_status))
        )
        new_response["reports"]["status"]["rejected"] = len(
            list(filter(lambda x: "rejected" in x, reports_status))
        )
        new_response["reports"]["status"]["submitted"] = len(
            list(filter(lambda x: "submitted" in x, reports_status))
        )
        new_response["reports"]["status"]["total"] = sum(
            new_response["reports"]["status"].values()
        )
        new_response["reports"]["status"] = {
            "labels": list(new_response["reports"]["status"].keys()),
            "data": list(new_response["reports"]["status"].values()),
        }
        ...
        new_response["milestones"]["time"] = {
            "labels": list(new_response["milestones"]["time"].keys()),
            "data": list(new_response["milestones"]["time"].values()),
        }

        new_response["milestones"]["status"] = {}
        new_response["milestones"]["status"]["open"] = len(
            [x for x in milestones_status if x not in ["completed", "rejected"]]
        )
        new_response["milestones"]["status"]["completed"] = len(
            list(filter(lambda x: "completed" in x, milestones_status))
        )
        new_response["milestones"]["status"]["total"] = (
            new_response["milestones"]["status"]["open"]
            + new_response["milestones"]["status"]["completed"]
        )
        new_response["milestones"]["status"] = {
            "labels": list(new_response["milestones"]["status"].keys()),
            "data": list(new_response["milestones"]["status"].values()),
        }
        ...
        new_response["issues"]["time"] = {
            "labels": list(new_response["issues"]["time"].keys()),
            "data": list(new_response["issues"]["time"].values()),
        }

        # Issue status
        new_response["issues"]["status"] = {}
        new_response["issues"]["status"]["open"] = len(
            list(filter(lambda x: "open" in x, issues_status))
        )
        new_response["issues"]["status"]["resolved"] = len(
            list(filter(lambda x: "resolved" in x, issues_status))
        )
        new_response["issues"]["status"]["total"] = (
            new_response["issues"]["status"]["open"]
            + new_response["issues"]["status"]["resolved"]
        )
        new_response["issues"]["status"] = {
            "labels": list(new_response["issues"]["status"].keys()),
            "data": list(new_response["issues"]["status"].values()),
        }

        # Natur of issue
        issues_nature = dict(Counter(issues_nature))
        new_response["issues"]["natureOfIssue"] = {
            "labels": list(issues_nature.keys()),
            "data": list(issues_nature.values()),
        }
        ...
        if "medium" in issues_criticality:
            new_response["issues"]["criticality"]["medium"] = issues_criticality[
                "medium"
            ]

        new_response["issues"]["criticality"] = {
            "labels": list(new_response["issues"]["criticality"].keys()),
            "data": list(new_response["issues"]["criticality"].values()),
        }
        ...
        (
            my_outstanding_documents,
            my_documents_overdue,
            my_documents_due_soon,
            my_documents_due_today,
            my_documents_status,
        ) = (0, 0, 0, 0, [])
        ...
        new_response["documents"]["user"]["status"]["requested"] = len(
            list(filter(lambda x: "requested" in x, my_documents_status))
        )
        new_response["documents"]["user"]["status"]["submitted"] = len(
            list(filter(lambda x: "submitted" in x, my_documents_status))
        )
        new_response["documents"]["user"]["status"]["completed"] = len(
            list(filter(lambda x: "completed" in x, my_documents_status))
        )
        new_response["documents"]["user"]["status"] = {
            "labels": list(new_response["documents"]["user"]["status"].keys()),
            "data": list(new_response["documents"]["user"]["status"].values()),
        }
        ...
        new_response["documents"]["user"]["time"] = {
            "labels": list(new_response["documents"]["user"]["time"].keys()),
            "data": list(new_response["documents"]["user"]["time"].values()),
        }
        ...
        new_response["documents"]["general"]["status"]["requested"] = len(
            list(filter(lambda x: "requested" in x, documents_status))
        )
        new_response["documents"]["general"]["status"]["submitted"] = len(
            list(filter(lambda x: "submitted" in x, documents_status))
        )
        new_response["documents"]["general"]["status"]["completed"] = len(
            list(filter(lambda x: "completed" in x, documents_status))
        )
        new_response["documents"]["general"]["status"] = {
            "labels": list(new_response["documents"]["general"]["status"].keys()),
            "data": list(new_response["documents"]["general"]["status"].values()),
        }
        ...
        new_response["documents"]["general"]["time"] = {
            "labels": list(new_response["documents"]["general"]["time"].keys()),
            "data": list(new_response["documents"]["general"]["time"].values()),
        }
    ...
    def get_analytics_overview(self, customer_id: str):
        ...
        # Define project expression to get specific keys in data
        projection_expression = ", ".join(["#status", "scopes", "dataroom"])

        expression_attribute_names = {"#status": "status"}

        data, _ = self._db.read_multiple_items(
        ...
        if data:
            ...
            document_status = {
                "labels": list(document_status.keys()),
                "data": list(document_status.values()),
            }
            ...
            issues_criticality = {
                "labels": list(issues_criticality.keys()),
                "data": list(issues_criticality.values()),
            }

            default_reports_status = {
                "pending": 0,
                "accepted": 0,
                "rejected": 0,
                "submitted": 0,
            ...
            reports_status = {
                "labels": list(reports_status.keys()),
                "data": list(reports_status.values()),
            }
            ...
            for milestone in milestones:
                if (
                    milestone
                    and datetime.strptime(milestone["startDate"], "%Y-%m-%d")
                    < datetime.today()
                ):
