...
class DiscussionsManager:
    ...
    def create_new_discussions(
        ...
        update_expression = (
            f"SET discussions.#discussionId = :{dynamo_object['discussionId']}"
        )
        expression_attribute_names = {"#discussionId": dynamo_object["discussionId"]}
        expression_attribute_values = {
            f":{dynamo_object['discussionId']}": dynamo_object
        }
        ...
        workflow = Workflows.update_workflows(
            token, "Create", message, project_id, dynamo_object["discussionId"]
        )
    ...
    def get_discussion_details(self, customer_id: str, project_id: str):
        ...
        if response:
            ...
            for i, discussion in enumerate(discussions):
                ...
                response, _ = self.get_previous_messages(
                    customer_id, discussion["discussionId"], "Discussions"
                )

                if response:

                    discussions[i]["lastMessage"] = sorted(
                        response, key=lambda x: x["timestamp"], reverse=False
                    )[-1]
    ...
    def update_discussion_details(
        ...
        for item in items:
            ...
            update_expression = "SET {}".format(
                ", ".join(
                    f"discussions.{discussions_id}.#{k}=:{k}" for k in item.keys()
                )
            )
            ...
            if message:
                workflow = Workflows.update_workflows(
                    token, "Update", message, project_id, discussions_id
                )
                self._db.create_item(f"Workflows-{customer_id}", workflow)

            if 200 <= http_status_code < 300:
                logger.info(
                    f"Discussion {discussions_id}'s details successfully updated, {http_status_code}"
                )
    ...
    def get_previous_messages(
        ...
        key = {"index_val": item_id, "index_name": index_name}
