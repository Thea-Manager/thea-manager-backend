...
class ScopeManager():
    ...
    @exception_handler
    def create_new_scope(self, token: str, object_id: str, customer_id: str, project_id: str, scope_name: str, start_date: str, end_date: str, consultant: dict, total_fees: str, billing_schedule: str, engagement_letter_ref: dict, team_members: list):
        """
        ...
        workflow = Workflows.update_workflows(token, "Create", message, project_id, dynamo_object["scopeId"])
    ...
    @exception_handler
    def update_scope_details(self, token, customer_id: str, project_id: str, items: list):
        """
        ...
        for item in items:
            ...
            update_expression = "SET {}".format(", ".join(f"scopes.{scope_id}.#{k}=:{k}" for k in item.keys()))
            ...
            if message:
                workflow = Workflows.update_workflows(token, "Update", message, project_id, scope_id)
                self._db.create_item(f"Workflows-{customer_id}", workflow)
    ...
    def delete_scope(self, customer_id: str, project_id: str, scopes: list):
        ...
        update_expression = "REMOVE {}".format(", ".join([f"scopes.{k}" for k in scopes]))
    ...
    @exception_handler
    def add_scope_members(self, token: str, customer_id: str, project_id: str, scope_id: str, team_members: list):
        """
        ...
        update_expression = "SET {}".format(", ".join([f"scopes.{scope_id}.teamMembers.#memberId_{i} = :{k['userId']}" for i, k in enumerate(team_members)]))   
        expression_attribute_names = {f"#memberId_{i}": k['userId'] for i, k in enumerate(team_members)}
        ...
        if message:
            workflow = Workflows.update_workflows(token, "Add", message, project_id, scope_id)
    ...
    @exception_handler
    def remove_scope_members(self, token: str, customer_id: str, project_id: str, scope_id: str, team_members: list):
        """
        ...
        update_expression = "REMOVE {}".format(", ".join([f"scopes.{scope_id}.teamMembers.{k['userId']}" for k in team_members]))
        self._db.update_item(table_name = table_name, key = key, update_expression = update_expression, return_values = "UPDATED_NEW")

        # Log workflow
        message = [f"removed {member['name']} ({member['email']})" for member in team_members]
        if message:
            workflow = Workflows.update_workflows(token, "Remove", message, project_id, scope_id)
            self._db.create_item(f"Workflows-{customer_id}", workflow)
