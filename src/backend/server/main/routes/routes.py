...
def general_exceptions(error):
    ...
    if "HTTP_" in str(error):
        return make_response(
            jsonify({"data": f"Missing {sub('HTTP_', '', str(error))} header"}), 500
        )
...
def scope_details(customerId: str, projectId: str, scopeId: str):
    ...
    kwargs = {"project_id": projectId, "customer_id": customerId, "scope_id": scopeId}
...
@api.route(
    "/milestones/<customerId>/<projectId>/<scopeId>/<milestoneId>", methods=["GET"]
)
# @verify_access_token
...
def milestones_overview(customerId: str, projectId: str, scopeId: str = ""):
    ...
    kwargs = {"customer_id": customerId, "project_id": projectId, "scope_id": scopeId}
...
def get_documents_overview(customerId: str, projectId: str, itemId: str = ""):
    ...
    kwargs = {"item_id": itemId, "project_id": projectId, "customer_id": customerId}
...
@api.route(
    "/dataroom/presigned/<customerId>/<projectId>/<itemId>/<filename>", methods=["GET"]
)
@api.route(
    "/dataroom/presigned/<customerId>/<projectId>/<itemId>/<filename>/<versionId>",
    methods=["GET"],
)
# @verify_access_token
def presigned_get(
    customerId: str, projectId: str, itemId: str, filename: str, versionId: str = ""
):
...
def user_details(organizationId: str, email: str):
    ...
    kwargs = {"organization_id": organizationId, "email": email}
