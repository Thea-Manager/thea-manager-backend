...
# Confirm signup
@exception_handler
def confirm_signup(client_id: str, client_secret: str, username: str, confirmation_code: str):
    """
...
def handler(event, context):
    ...
    # Sign up
    response, code = confirm_signup(client_id, client_secret, username, confirmation_code)
    
    if code == 200:
    
        # Store user details to Dynamodb
        response, code = store_user_details(f"users-{user_attributes['custom:orgId']}", user_attributes)
