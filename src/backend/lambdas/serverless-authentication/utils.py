    ...
    def inner_function(*args, **kwargs):
        try:
            ...
            if type(e).__name__ == "InternalErrorException":
                ...
                logger.error(
                    "LimitExceededException - Too many signin attempts, try after sometime"
                )
                return (
                    "Too many signin attempts, try after sometime",
                    e.response["ResponseMetadata"]["HTTPStatusCode"],
                )
                ...
                return (
                    sub(r"[1-9]*[a-zA-Z]+[1-9]+", "", e.args[0]),
                    e.response["ResponseMetadata"]["HTTPStatusCode"],
                )
            elif type(e).__name__ == "ResourceNotFoundException":
                logger.error(
                    "ResourceNotFoundException - Invalid client ID, user pool client doesn't exist"
                )
                return (
                    "Invalid client ID, user pool client doesn't exist",
                    e.response["ResponseMetadata"]["HTTPStatusCode"],
                )
                ...
                logger.error(
                    "UnexpectedLambdaException - Unexpected exception with AWS Lambda service"
                )
                return (
                ...
                return (
                    "Password reset requried",
                    e.response["ResponseMetadata"]["HTTPStatusCode"],
                )
            elif type(e).__name__ == "SoftwareTokenMFANotFoundException":
                logger.error(
                    "SoftwareTokenMFANotFoundException - Software TOTP MFA not enabled for user pool"
                )
                return (
                ...
                logger.error(
                    "InvalidEmailRoleAccessPolicyException - Amazon Cognito is not allowed to use your email identity"
                )
                return (
                ...
                logger.error(
                    "CodeDeliveryFailureException - Verification code failes to delivery successfully"
                )
                return (
                    "Verification code failed to be successfully delivered",
                    e.response["ResponseMetadata"]["HTTPStatusCode"],
                )
            elif type(e).__name__ == "ConcurrentModificationException":
                logger.error(
                    "ConcurrentModificationException - multiple modification happening concurrently"
                )
                return (
                    "Concurrent modifications occuring",
                    e.response["ResponseMetadata"]["HTTPStatusCode"],
                )
...
def compute_secret_hash(client_id: str, client_secret: str, username: str):
    ...
    dig = new(
        client_secret.encode("UTF-8"), msg=message.encode("UTF-8"), digestmod=sha256
    ).digest()
