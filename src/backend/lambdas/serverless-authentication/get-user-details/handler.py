...
def handler(event, context):
    ...
    if response["ResponseMetadata"]["HTTPStatusCode"] == 200:
        ...
        user_attributes_1 = {
            sub("custom:", "", x["Name"]): x["Value"]
            for x in response["UserAttributes"]
        }

        user_attributes_2, code = get_user_details_dynamo(
            user_attributes_1["email"], user_attributes_1["orgId"]
        )
    ...
    return {"data": response, "statusCode": code, "authValid": auth_valid}
