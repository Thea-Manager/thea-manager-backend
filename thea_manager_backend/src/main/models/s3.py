...
class S3():
    ...
    @exception_handler
    def create_presigned_url(self, bucket_name: str, object_name: str, version_id: str = "", upload: bool = False, expiration: int = 600, metadata: dict = None):
        """
        ...
        if not upload:
            ...
            if not version_id:
                ...
                target_params = {"Bucket": bucket_name, "Key": object_name, "VersionId": version_id}
            response = self._s3_client.generate_presigned_url("get_object", Params = target_params, ExpiresIn = expiration)
        else:
            if metadata:
    ...
    @exception_handler
    def list_files(self, bucket_name: str, prefix: str, page_size: int = 50, max_keys: int = 10000, starting_token: str = None):
        """
    ...
    def list_file_versions(self, bucket_name: str, prefix: str):
        ...
        # Type guarding
        assert check_argument_types()
        response = self._s3_client.list_object_versions(Bucket = bucket_name, Prefix = prefix)
        return response["Versions"], response['ResponseMetadata']['HTTPStatusCode']

    @exception_handler
    def get_object_metadata(self, bucket_name: str, object_name: str, version_id: str = None):
        """
        ...
        return response["ResponseMetadata"]["HTTPHeaders"], response["ResponseMetadata"]["HTTPStatusCode"]
