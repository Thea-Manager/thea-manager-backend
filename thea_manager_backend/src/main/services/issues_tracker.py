...
class IssuesTracker():
    ...
    @exception_handler
    def create_new_issue(self, token: str, object_id: str, customer_id: str, project_id: str, scope_id: str, issue_name: str, region: str, business_unit: str, date_of_raise: str, due_date: str, nature_of_issue: str, criticality: str, issue_description: str, impact_value: str, currency: str, impact_on: str, document_ref: dict, issue_owner: dict, resolution_path: str):
        """
        ...
        update_expression = f"SET scopes.#scopeId.issues.#IssueId = :{dynamo_object['issueId']}"
        expression_attribute_names = {"#IssueId": dynamo_object['issueId'], "#scopeId": scope_id}
        ...
        workflow = Workflows.update_workflows(token, "Create", message, project_id, dynamo_object["issueId"])
    ...
    @exception_handler
    def get_issue_details(self, customer_id: str, project_id: str, scope_id: str, issues_id: str):
        """
    ...
    @exception_handler
    def get_issues_overview(self, customer_id: str, project_id: str, scope_id: str = ""):
        """
    ...
    @exception_handler
    def update_existing_issue(self, token: str, customer_id: str, project_id: str, items: list):
        """
        ...
        for item in items:
            ...
            update_expression = "SET {}".format(", ".join(f"scopes.{scope_id}.issues.{issues_id}.#{k}=:{k}" for k in item.keys()))
            ...
            if message:
                workflow = Workflows.update_workflows(token, "Update", message, project_id, issues_id)
                self._db.create_item(f"Workflows-{customer_id}", workflow)


            if 200 <= http_status_code < 300:
                logger.info(f"Issue {issues_id}'s details successfully updated, {http_status_code}")
    ...
    @exception_handler
    def delete_existing_issues(self, token: str, customer_id: str, project_id: str, issues: list):
        """
        ...
        update_expression = "REMOVE {}".format(", ".join([f"issues.{k}" for k in issues]))
        ...
        for issue_id in issues:
            ...
            workflow = Workflows.update_workflows(token, "Delete", message, project_id, issue_id)
            self._db.create_item(f"Workflows-{customer_id}", workflow)
