...
class ProjectsManager():
    ...
    @exception_handler
    def create_new_project(self, token: str, object_id: str, customer_id: str, project_name: str, project_type: str, type: str, business_unit: str, internal_project_owner: dict, internal_client_lead: dict, lead_consulting_partner: list, consulting_companies: str, start_date: str, estimated_end_date: str, budgeted_cost: str, currency: str, team_members: list, linked_projects: list = None, overwrite_generate_code: str = ""):
        """
        ...
        workflow = Workflows.update_workflows(token, "Create", message, dynamo_object["projectId"], dynamo_object["projectId"])
    ...
    def get_project_information(self, customer_id: str, project_id: str):
        ...
        response, http_status_code = self._db.read_single_item(table_name, key, projection_expression, expression_attribute_names)
        ...
        if isinstance(response, dict):
            if "scopes" in response.keys():
                ...
                for i in range(len(response["scopes"])):
                    response["scopes"][i]["issues"] = list(response["scopes"][i]["issues"].values())
                    response["scopes"][i]["reports"] = list(response["scopes"][i]["reports"].values())
                    response["scopes"][i]["dataroom"] = list(response["scopes"][i]["dataroom"].values())
                    response["scopes"][i]["milestones"] = list(response["scopes"][i]["milestones"].values())
                    response["scopes"][i]["teamMembers"] = list(response["scopes"][i]["teamMembers"].values())

        today_date, due_date = datetime.today(), datetime.strptime(response["endDate"], "%Y-%m-%d")
    ...
    def get_project_overview(self, customer_id: str, last_evaluated_key: str = None):
        ...
        projects, code = self._db.read_multiple_items(table_name, key, projection_expression, expression_attribute_names, last_evaluated_key)

        for i in range(len(projects)):

            today_date, due_date = datetime.today(), datetime.strptime(projects[i]["endDate"], "%Y-%m-%d")
    ...
    @exception_handler
    def update_project_info(self, token: str, customer_id: str, project_id: str, item: dict):
        """
        ...
        expression_attribute_names = {"#status": "status", "#type":"type"}
        logger.info(f"Checking if project ID or organization ID exists: {key}")
        previous_item, _ =  self._db.read_single_item(table_name, key, projection_expression, expression_attribute_names)
        ...
        if message:
            workflow = Workflows.update_workflows(token, "Update", message, project_id, project_id)
            self._db.create_item(f"Workflows-{customer_id}", workflow)
    ...
    @exception_handler
    def add_members(self, token: str, customer_id: str, project_id: str, team_members: list):
        """
        ...
        update_expression = "SET {}".format(", ".join([f"teamMembers.#userId_{i} = :{k['userId']}" for i, k in enumerate(team_members)]))   
        expression_attribute_names = {f"#userId_{i}": k['userId'] for i, k in enumerate(team_members)}
        ...
        # Log workflow
        message = [f"Added {member['name']} ({member['email']})" for member in team_members]
        if message:
            workflow = Workflows.update_workflows(token, "Add", message, project_id, project_id)
            self._db.create_item(f"Workflows-{customer_id}", workflow)
    ...
    @exception_handler
    def remove_members(self, token: str, customer_id: str, project_id: str, team_members: list):
        """
